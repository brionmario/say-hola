...
export function TabBar({primary, secondary = [], showTabMarkers = true, side, translucent = false, children}) {
  return (
    <div className="flex text-slate-400 text-xs leading-6">
      ...
      >
        {secondary.map(({name, open = true, className}) => (
          <div key={name} className={cn('px-4 py-1 border-r border-slate-200/5', className, {italic: !open})}>
            {name}
          </div>
        ))}
        {children && <div className="flex-auto flex items-center justify-end px-4 space-x-4">{children}</div>}
      </div>
    </div>
  );
}
